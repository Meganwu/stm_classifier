...
def transfer_directory(current_time=None, model='sac', episodes=130, diss_maxtime=10, succ=0, succ_diss=0):
    ...
    for episode_i in range(episodes):
        for diss_i in range(diss_maxtime):
            ...
            with open(os.path.join(save_all_diss_name, 'all_tip_pos_data_label.txt'), 'a') as f:
                f.write('%d, %d, %s\n' % (episode_i, diss_i, vert_tip_pos[-1]))

            if vert_tip_pos[-1]==True:
                succ_diss+=1
...
def plot_all_diss(current_time=None, model='sac', episodes=130, diss_maxtime=10, succ=0, column_num=4):
    plt.figure(figsize=(15,650))
    all_tip_pos=[]
    all_diss_data=[]
    for episode_i in range(episodes):

        plt.subplot(episodes*diss_maxtime,column_num,(succ+1)*column_num+1)
        ...
        for diss_i in range(diss_maxtime):
            ...
            plt.subplot(episodes*diss_maxtime,column_num,succ*column_num+3)
            ...
            plt.subplot(episodes*diss_maxtime,column_num,succ*column_num+4)
            ...
            plt.subplot(episodes*diss_maxtime,column_num,succ*column_num+2)
