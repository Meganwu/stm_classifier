...
scanmandata = namedtuple('scanmandata',['img_forward', 'img_backword', 'offset_nm', 'len_nm', 'current','dI_dV'])
latmandata = namedtuple('latmandata',['time','x','y','current','dI_dV','topography'])
dissmandata = namedtuple('dissmandata',['time','V','Z','current','dI_dV','topography'])
dissassmandata = namedtuple('dissassmandata',['time','V','Z','current','dI_dV','topography'])
#assmandatadissassmandata = namedtuple('assmandata',['time','V','Z','current','dI_dV','topography'])
...
class Createc_Controller:
    ...
    def scan_image(self,
        ...
        img_forward = np.array(self.stm.scandata(1,4))   # it is topography
        img_backward = np.array(self.stm.scandata(257,4))
        current_scan = np.array(self.stm.scandata(2,3))
        dI_dV_scan = np.array(self.stm.scandata(6,0))
        ...
        data = scanmandata(img_forward, img_backward, self.offset_nm, np.array([x_length, y_length]), current_scan, dI_dV_scan)
        return data
    ...
    def diss_manipulation(self, 
        ...
        '''implement the dissociation or association action, and collect the data'''
    



        # offset_nm=self.offset_nm
        # len_nm=self.im_size_nm 
        ...
        args = diss_x_nm, diss_y_nm, x_end_nm, y_end_nm, offset_nm, len_nm
        rets = self.nm_to_pixel(*args)
        x_start_pixel, y_start_pixel, x_end_pixel, y_end_pixel = rets

        
        self.set_Z_approach(diss_z_nm)
        args = diss_x_nm, diss_y_nm, x_end_nm, y_end_nm, offset_nm, len_nm 
        ...
        self.stm.btn_vertspec(x_start_pixel, y_start_pixel)  #unit: image pixel
        self.stm.waitms(50)
        ...
        data = dissassmandata(time, V, Z, current, dI_dV, topography)
        return data
    ...
    def tip_form(self, A: float, x_nm: float, y_nm: float, tipform_offset_nm: float, tipform_len_nm: float) -> None:
        """Perform tip forming
        ...
        tipform_offset_nm : np.array
            STM tip form section offset value in nm

        tipform_len_nm : float
            STM tip form section length in nm
        ...
        Xpiezoconst = float(self.stm.getparam("Xpiezoconst"))
        Ypiezoconst = float(self.stm.getparam("Ypiezoconst"))
        self.stm.setxyoffvolt(GainX*tipform_offset_nm[0]/Xpiezoconst,GainX*tipform_offset_nm[1]/Ypiezoconst)

        Delta_X = self.get_Delta_X(tipform_len_nm)
        self.stm.setparam('Delta X [Dac]',Delta_X)

        self.set_Z_approach(A)
        args = x_nm, y_nm, None, None, tipform_offset_nm, tipform_len_nm
        x_pixel, y_pixel, _, _ = self.nm_to_pixel(*args)
        self.stm.btn_tipform(x_pixel, y_pixel)
        self.stm.waitms(30000)
